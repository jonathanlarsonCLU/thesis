...
dbtst_overall_raw <- read_excel("larson(overall_raw_ma).xlsx")
View(dbtst_overall_raw)

# study id, author, year, outcome, test, treatment n, control n, hedge's g, variance of hedge's g, p-value of hedge's g
dbtst_overall_es <- read_excel("larson(overall_es_ma).xlsx")
View(dbtst_overall_es)


```


# Calculate effect sizes of overall data
```{r}

res1 <- mes(m.1 = m1T, m.2 = m1C, sd.1 = sd1T, sd.2 = sd1C, n.1 = nT, n.2 = nC, id = id, data = dbtst_overall_raw)
...
# Aggregate dependent effect sizes of overall data
```{r}
...
dbtst_agg <- agg(id = id, es = g, var = var.g, n.1 = nT, n.2 = nC, cor = 0.5, method = "BHHR", data = dbtst_overall_es)
...
# Estimate summary effect by random-effects omnibus test of overall data
```{r}
...
# Forest plot of all outcomes of overall data
```{r}
...
# Evaluate heterogeneity of summary effect of overall data
```{r}
...
# Funnel plot of all outcomes of overall data
```{r}
